...
print(f"Msg1: state={r1['workflow_state']}, intent={r1['intent']}, type={r1.get('session_type')}")

r2 = client.post(f"/api/sessions/{sid}/message", json={"message": "My name is Alice Smith"}).json()
print(f"Msg2: state={r2['workflow_state']}, intent={r2['intent']}, type={r2.get('session_type')}, name={r2['collected_data'].get('full_name')}")
